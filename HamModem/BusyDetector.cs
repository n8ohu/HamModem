...
namespace HamModem
{
	public class BusyDetector
	{
		...
		public int BusyDetect(ref double[] dblMag, int intStart, int intStop)
		{
			...
			if (!(MainClass.State == MainClass.TestState.SearchingForLeader | MainClass.State == MainClass.TestState.AcquireSymbolSync | MainClass.State == MainClass.TestState.AcquireFrameSync))
